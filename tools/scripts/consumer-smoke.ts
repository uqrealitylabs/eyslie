...
for (const [index, version] of versions.entries()) {
  ...
  writeFileSync(
    ...
    [
      ...
      'import { LivingText, livingTextMoods, livingTextThemes } from "@uqrealitylabs/eyslie";',
      "const cases = [",
  ...
  if (isLatest) {
    ...
    execFileSync(
      ...
      [
        ...
        "--input-type=module",
        "-e",
        'const p = await import("@uqrealitylabs/eyslie"); if (p.splitTextLetters("O🙂").length !== 2 || p.parseEyeMarkers("^O").text !== "O" || !p.isPointerNear({ left: 0, top: 0, right: 1, bottom: 1 }, { x: 0, y: 0 }, 0)) throw new Error("server helpers unavailable");',
      ],
      { cwd: consumer, stdio: "inherit" },
    );
    for (const entry of [
